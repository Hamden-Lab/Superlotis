...
// void write_fits_header(std::ofstream& fits_file) {
//     const int card_size = 80;
//     const int max_cards = 32;
//     char cards[max_cards][card_size];
//     int card_count = 0;

//     auto add_card = [&](const char* key, const char* value, const char* comment = nullptr) {
//         char card[card_size + 1];
//         std::memset(card, ' ', card_size);
//         card[card_size] = '\0';

//         char formatted[card_size + 1];
//         if (comment && comment[0] != '\0') {
//             std::snprintf(formatted, card_size + 1, "%-8s= %20s / %s", key, value, comment);
//         } else {
//             std::snprintf(formatted, card_size + 1, "%-8s= %20s", key, value);
//         }

//         std::memcpy(card, formatted, std::strlen(formatted));
//         std::memcpy(cards[card_count++], card, card_size);
//     };

//     char width_str[16], height_str[16];
//     std::snprintf(width_str, sizeof(width_str), "%d", params.res_width);
//     std::snprintf(height_str, sizeof(height_str), "%d", params.res_height);

//     add_card("SIMPLE", "T", "Standard FITS format");
//     add_card("BITPIX", "16", "16-bit signed integer");
//     add_card("NAXIS", "2", "Number of axes");
//     add_card("NAXIS1", width_str, "Width");
//     add_card("NAXIS2", height_str, "Height");
//     add_card("END", "");

//     for (int i = 0; i < card_count; ++i) {
//         fits_file.write(cards[i], card_size);
//     }

//     int total_bytes = card_count * card_size;
//     int pad = 2880 - (total_bytes % 2880);
//     if (pad != 2880) {
//         char* padding = (char*)std::calloc(pad, 1);
//         fits_file.write(padding, pad);
//         std::free(padding);
//     }
// }

// void write_fits_data(std::ofstream& fits_file, const char* data, int num_pixels) {
//     const int element_size = 2;
//     int total_bytes = num_pixels * element_size;

//     fits_file.write(data, total_bytes);
...
//     write_fits_header(fits_file, params.res_width, params.res_height);
//     write_fits_data(fits_file, data, params.res_width * params.res_height);
...
// int main() {
//     std::string filename = "example.raw"; 
//     params.res_width = 2048; // Example width
//     params.res_height = 2048; // Example height

//     piint result = resize_raw(filename.c_str());
//     if (result == 0) {
