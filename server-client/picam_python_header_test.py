...
class Camera():
    def __init__(self,*,libPath: str='C:/opt/PrincetonInstruments/picam/samples/Picam.dll'):#C:/Program Files/Princeton Instruments/PICam/Runtime/Picam.dll'):	#class will instantiate and initialize PICam
...
#                print('  Model is ', pit.PicamModelLookup[self.camIDs[i].model])
    ...
    def Acquisition(self, N=1, timeout=100000):
        ...
        print("Duration of acquision : %f s" % (time.time() - t) )
#        print(errors)
        return t
...
#        print(running.value)
    ...
    def GetAcquiredData(self):
        ...
        print(self.totalFrameSize)
        print(int(self.totalFrameSize))
        print(ctypes.c_uint16 * int(self.totalFrameSize))
        dataArrayType = ctypes.c_uint16 * int(self.totalFrameSize) #readoutstride
        # print(dataArrayType)
        dataArrayPointerType = ctypes.POINTER(dataArrayType)
        # print(dataArrayPointerType)
        ...
        # return self.available.initial_readout
...
if __name__ == "__main__":
    try:
        ...
        print("Open camera")
        # Open and initialize the first available camera
        camera.OpenFirstCamera()
        
        # Get and print initial camera settings
        print("\nInitial Camera Settings:")
        print(camera.getSerialNumber())

        print(f"Temperature: {camera.GetTemperature():.2f}°C")
        print(f"Exposure Time: {camera.GetExposure():.3f} seconds")
        print(camera.getAvailableCameras())
        print(f"ROI: {camera.setROI(0, 2048, 1, 0, 2048, 1, 1)}")

        # print(f"First ROI: {camera.GetFirstROI():.3f} seconds")
        ...
        # camera.SetAdcConf(gain=3, speed=1.0)
        
        # Parameters: x0, width, xbin, y0, height, ybin, store
        # camera.setROI(0, 1024, 1, 0, 1024, 1, 1)
        
        # Take a single acquisition
        # print("\nAcquiring image...")
        # camera.Acquisition(N=2)
        
        # # Get the acquired data
        # image_data = camera.GetAcquiredData()
        # print(f"Image acquired: Shape = {image_data.shape}")
        # print(np.mean(image_data))
        if camera.IsAcquisitionRunning() is False:
                camera.Acquisition(N=1,timeout=120000)
                print('----->  Acquisition ')
                try :
                    data = camera.GetAcquiredData()
    ...
    # finally:
    #     # Always disconnect the camera properly
...
#     camera.OpenFirstCamera()
#     # print(f"Camera Sensor: {camera.camID.sensor_name.decode('utf-8')}, Serial #: {camera.camID.serial_number.decode('utf-8')}")

#     #     # Set exposure time
#     #     camera.SetExposure(1000.0)
#     #     print(f"Exposure time set to: {camera.GetExposure()}")

#     #     # Get the temperature of the sensor
#     #     temperature = camera.GetTemperature()
#     #     print(f"Sensor Temperature: {temperature}")

#     #     # Acquire an image
#     #     camera.Acquisition(N=1)
#     #     image_data = camera.GetAcquiredData()
#     #     print(f"Acquired image data: {image_data}")
